...
class ConceptsController(BaseController):
    def __init__(self, loader, state=None, mutex=None, parent=None):
        super().__init__(loader, "modules/ui/views/tabs/concepts.ui", state=state, mutex=mutex, name=QCA.translate("main_window_tabs", "Concepts"), parent=parent)

        self.concept_window = WinConceptController(loader, state=state, mutex=mutex, parent=self)
        self.children = {}
    ...
    def __appendConcept(self):
        wdg = WidgetConceptController(self.loader, self.concept_window, len(self.children), state=self.state, mutex=self.mutex, parent=self)
